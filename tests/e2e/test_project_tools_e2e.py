...
async def test_update_project_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        ...
        assert (
            update_result.data["description"]
        ...
        assert (
            get_result.data["description"] == "Updated description with new information"
        )
...
async def test_list_projects_filter_by_status_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        ...
        archived_result = await client.call_tool(
            "execute_forgetful_tool",
...
async def test_list_projects_filter_by_repo_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        ...
        forgetful_result = await client.call_tool(
            "execute_forgetful_tool",
            {
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_list_projects_filter_by_name_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        ...
        uppercase_result = await client.call_tool(
            "execute_forgetful_tool",
            {"tool_name": "list_projects", "arguments": {"name": "FORGETFUL"}},
        )
