...
async def test_create_project_basic_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        result = await client.call_tool('execute_forgetful_tool', {
            ...
            'forgetful-e2e', 'description':
            'MIT-licensed memory service implementing atomic memory principles'
            , 'project_type': 'development', 'status': 'active',
            'repo_name': 'scottrbk/forgetful', 'notes':
...
async def test_get_project_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        create_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'create_project', 'arguments': {'name':
            'test-get-project', 'description': 'Test project for retrieval',
            'project_type': 'work'}})
        project_id = create_result.data["id"]
        get_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'get_project', 'arguments': {'project_id':
            project_id}})
    ...
    async with Client(mcp_server_url) as client:
        ...
        for name in project_names:
            await client.call_tool('execute_forgetful_tool', {'tool_name':
                'create_project', 'arguments': {'name': name, 'description':
                f'Description for {name}', 'project_type': 'development'}})
...
async def test_update_project_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        create_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'create_project', 'arguments': {'name':
            'original-name', 'description': 'Original description',
            'project_type': 'development'}})
        project_id = create_result.data["id"]
        update_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'update_project', 'arguments': {'project_id':
            project_id, 'name': 'updated-name', 'description':
...
async def test_delete_project_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        create_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'create_project', 'arguments': {'name':
            'to-delete', 'description': 'This project will be deleted',
            'project_type': 'development'}})
        project_id = create_result.data["id"]
        delete_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'delete_project', 'arguments': {'project_id':
            project_id}})
        ...
        with pytest.raises((ToolError, Exception)) as exc_info:
            result = await client.call_tool('execute_forgetful_tool', {'tool_name':
                'get_project', 'arguments': {'project_id': project_id}})
            if result.data is None or (hasattr(result, 'is_error') and result.is_error):
                raise ValueError("Project not found")
        error_message = str(exc_info.value).lower()
        assert 'not found' in error_message or 'validation_error' in error_message or 'project not found' in error_message


@pytest.mark.e2e
@pytest.mark.asyncio
async def test_list_projects_filter_by_status_e2e(docker_services,
    mcp_server_url):
    """Test filtering projects by status"""
    async with Client(mcp_server_url) as client:
        await client.call_tool('execute_forgetful_tool', {'tool_name':
            'create_project', 'arguments': {'name': 'active-status-test',
            'description': 'Active project', 'project_type': 'development',
            'status': 'active'}})
        await client.call_tool('execute_forgetful_tool', {'tool_name':
            'create_project', 'arguments': {'name': 'archived-status-test',
            'description': 'Archived project', 'project_type':
        ...
        archived_result = await client.call_tool('execute_forgetful_tool',
...
async def test_list_projects_filter_by_repo_e2e(docker_services, mcp_server_url
    ...
    async with Client(mcp_server_url) as client:
        await client.call_tool('execute_forgetful_tool', {'tool_name':
            'create_project', 'arguments': {'name': 'forgetful-repo-test',
            'description': 'Forgetful project', 'project_type':
            'development', 'repo_name': 'scottrbk/forgetful'}})
        await client.call_tool('execute_forgetful_tool', {'tool_name':
            'create_project', 'arguments': {'name': 'other-repo-test',
            'description': 'Other project', 'project_type': 'development',
            'repo_name': 'scottrbk/other-repo'}})
        forgetful_result = await client.call_tool('execute_forgetful_tool',
            {'tool_name': 'list_projects', 'arguments': {'repo_name':
            'scottrbk/forgetful'}})
...
async def test_project_persistence_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        create_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'create_project', 'arguments': {'name':
            'persistence-test', 'description': 'Test project persistence',
            'project_type': 'development'}})
        project_id = create_result.data["id"]
        for _ in range(3):
            get_result = await client.call_tool('execute_forgetful_tool', {
...
async def test_get_project_invalid_id_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        with pytest.raises((ToolError, Exception)) as exc_info:
            result = await client.call_tool('execute_forgetful_tool', {'tool_name':
                'get_project', 'arguments': {'project_id': 999999}})
...
async def test_update_project_invalid_id_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        with pytest.raises((ToolError, Exception)) as exc_info:
            result = await client.call_tool('execute_forgetful_tool', {'tool_name':
                'update_project', 'arguments': {'project_id': 999999,
...
async def test_delete_project_invalid_id_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        result = await client.call_tool('execute_forgetful_tool', {'tool_name':
            'delete_project', 'arguments': {'project_id': 999999}})
...
@pytest.mark.e2e
@pytest.mark.asyncio
async def test_create_project_validation_error_e2e(docker_services,
    mcp_server_url):
    """Test validation error with invalid repo_name format"""
    async with Client(mcp_server_url) as client:
        try:
            await client.call_tool('execute_forgetful_tool', {'tool_name':
                'create_project', 'arguments': {'name': 'invalid-repo',
                'description': 'Project with invalid repo format',
                'project_type': 'development', 'repo_name':
...
async def test_update_project_partial_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        create_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'create_project', 'arguments': {'name':
            'patch-test', 'description': 'Original description',
            'project_type': 'development', 'repo_name': 'scottrbk/test-repo'}})
        project_id = create_result.data["id"]
        update_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'update_project', 'arguments': {'project_id':
            project_id, 'name': 'updated-patch-test'}})
...
async def test_update_project_archive_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        create_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'create_project', 'arguments': {'name':
            'archive-test', 'description': 'Project to archive',
            'project_type': 'development', 'status': 'active'}})
        project_id = create_result.data["id"]
...
async def test_list_returns_summary_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        create_result = await client.call_tool('execute_forgetful_tool', {
            ...
            'This is a long description that should not appear in list',
            'project_type': 'development', 'notes':
            'These are notes that should not appear in list'}})
        ...
        project_dict = our_project.__dict__ if hasattr(our_project, '__dict__'
            ) else our_project
...
async def test_get_returns_full_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        create_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'create_project', 'arguments': {'name':
            'full-test', 'description': 'This is the full description',
            'project_type': 'development', 'notes':
            'These are the full notes'}})
        project_id = create_result.data["id"]
        get_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'get_project', 'arguments': {'project_id':
            project_id}})
...
async def test_project_memory_count_updates_e2e(docker_services, mcp_server_url
    ...
    async with Client(mcp_server_url) as client:
        project_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'create_project', 'arguments': {'name':
            'memory-count-test', 'description': 'Test memory_count updates',
            'project_type': 'development'}})
        project_id = project_result.data["id"]
        assert project_result.data["memory_count"] == 0
        for i in range(3):
            await client.call_tool('execute_forgetful_tool', {'tool_name':
                'create_memory', 'arguments': {'title':
                f'Memory {i} for project', 'content':
                f'This is memory {i} linked to the project', 'context':
                'Testing memory count', 'keywords': ['test', 'memory',
                'count'], 'tags': ['test'], 'importance': 7, 'project_ids':
                [project_id]}})
        get_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'get_project', 'arguments': {'project_id':
            project_id}})
...
async def test_query_memory_by_project_e2e(docker_services, mcp_server_url):
    ...
    async with Client(mcp_server_url) as client:
        project1_result = await client.call_tool('execute_forgetful_tool',
            {'tool_name': 'create_project', 'arguments': {'name':
            'query-project-1', 'description':
            'First project for query test', 'project_type': 'development'}})
        project2_result = await client.call_tool('execute_forgetful_tool',
            {'tool_name': 'create_project', 'arguments': {'name':
            'query-project-2', 'description':
            'Second project for query test', 'project_type': 'development'}})
        project1_id = project1_result.data["id"]
        project2_id = project2_result.data["id"]
        for i in range(2):
            await client.call_tool('execute_forgetful_tool', {'tool_name':
                'create_memory', 'arguments': {'title':
                f'Project 1 memory {i}', 'content':
                f'Content for project 1 memory {i}', 'context':
                'Testing project filtering', 'keywords': ['project1',
                'test'], 'tags': ['test'], 'importance': 7, 'project_ids':
                [project1_id]}})
        for i in range(2):
            await client.call_tool('execute_forgetful_tool', {'tool_name':
                'create_memory', 'arguments': {'title':
                f'Project 2 memory {i}', 'content':
                f'Content for project 2 memory {i}', 'context':
                'Testing project filtering', 'keywords': ['project2',
                'test'], 'tags': ['test'], 'importance': 7, 'project_ids':
                [project2_id]}})
        query_result = await client.call_tool('execute_forgetful_tool', {
            'tool_name': 'query_memory', 'arguments': {'query': 'memory',
            'query_context': 'Looking for project 1 memories', 'k': 10,
