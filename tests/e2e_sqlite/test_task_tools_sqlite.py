...
async def test_create_task_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-create-proj")
...
async def test_get_task_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-get-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
...
async def test_query_tasks_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-query-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
...
async def test_update_task_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-update-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
...
async def test_transition_task_lifecycle_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-lifecycle-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
...
async def test_transition_task_invalid_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-invalid-trans-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
...
async def test_claim_task_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-claim-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
...
async def test_claim_task_version_conflict_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-conflict-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
...
async def test_criteria_lifecycle_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-criteria-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
...
async def test_done_requires_criteria_met_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-criteria-gate-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
...
@pytest.mark.asyncio
async def test_done_after_criteria_met_e2e(mcp_client):
    """Test full flow: add criterion -> verify -> transition to done"""
    project_id = await _create_project(mcp_client, "task-criteria-done-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
    ...
    create_result = await mcp_client.call_tool(
        ...
        {
            "tool_name": "create_task",
            "arguments": {"plan_id": plan_id, "title": "Criteria then done"},
        },
    )
    task_id = create_result.data["id"]
    version = create_result.data["version"]
    ...
    # Add criterion
    crit_result = await mcp_client.call_tool(
        "execute_forgetful_tool",
        {
            "tool_name": "add_criterion",
            "arguments": {"task_id": task_id, "description": "Code reviewed"},
        },
    )
    criterion_id = crit_result.data["id"]
    ...
    await mcp_client.call_tool(
        ...
        {
            "tool_name": "verify_criterion",
            "arguments": {"criterion_id": criterion_id, "met": True},
        },
    )
...
async def test_dependency_chain_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-dep-chain-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
...
async def test_cyclic_dependency_rejected_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-cycle-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
...
async def test_remove_dependency_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-remove-dep-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
...
async def test_plan_auto_completes_when_all_tasks_done_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-auto-complete-proj")
...
async def test_create_task_with_inline_criteria_e2e(mcp_client):
    ...
    project_id = await _create_project(mcp_client, "task-inline-crit-proj")
    plan_id = await _create_active_plan(mcp_client, project_id)
