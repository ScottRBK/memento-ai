"""Plan repository for SQLite data access operations"""
...
from app.repositories.sqlite.sqlite_tables import PlansTable

logger = logging.getLogger(__name__)


class SqlitePlanRepository:
    """Repository for Plan entity operations in SQLite."""
    ...
    async def create_plan(self, user_id: UUID, plan_data: PlanCreate) -> Plan:
        ...
        async with self.db_adapter.session(user_id) as session:
            ...
            new_plan = PlansTable(**data, user_id=str(user_id))
    ...
    async def get_plan_by_id(self, user_id: UUID, plan_id: int) -> Plan | None:
        ...
        async with self.db_adapter.session(user_id) as session:
            stmt = (
                ...
                .where(PlansTable.user_id == str(user_id), PlansTable.id == plan_id)
            )
    ...
    async def list_plans(
        ...
        async with self.db_adapter.session(user_id) as session:
            stmt = (
                select(PlansTable)
                .options(selectinload(PlansTable.tasks))
                .where(PlansTable.user_id == str(user_id))
            )
    ...
    async def delete_plan(self, user_id: UUID, plan_id: int) -> bool:
        ...
        async with self.db_adapter.session(user_id) as session:
            stmt = delete(PlansTable).where(
                PlansTable.user_id == str(user_id), PlansTable.id == plan_id
            )
            result = await session.execute(stmt)
